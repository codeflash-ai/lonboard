...
class NormalAccessor(FixedErrorTraitType):
    ...
    def _numpy_to_arrow(self, obj: BaseArrowLayer, value: np.ndarray) -> ChunkedArray:
        ...
        # Only warn and convert if dtype is not already float32
    ...
    def validate(
        ...
        if not DataType.is_floating(field_type):
            self.error(
                obj,
                value,
        ...
        # Only cast if value_type is not already float32 FixedSizeList
        target_type = DataType.list(Field("", DataType.float32()), 3)
